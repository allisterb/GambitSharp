...
public class Program : Runtime
{
    ...
    {
        Initialize("GambitSharp", "CLI", (args.Contains("--debug") || args.Contains("-d")), true, true);
        PrintLogo();
    ...
    static void Help(ParserResult<object> result, IEnumerable<Error> errors)
    {
        ...
        help.Heading = new HeadingInfo("GambitSharp generator command-line help");
        help.Copyright = "";
        if (errors.Any(e => e.Tag == ErrorType.VersionRequestedError))
        {
            help.Heading = new HeadingInfo("GambitSharp", AssemblyVersion.ToString(3));
            help.Copyright = "";
            Info(help);
    ...
    static void Generate(GenerateOptions go)
    {
        ...
        var bindingspath = Path.Combine(go.LibPath, "..", "..", "src", "GambitSharp.Bindings");
    ...
    {
        Con.Write(new FigletText(font, "GambitSharp").Color(Color.Pink1));
